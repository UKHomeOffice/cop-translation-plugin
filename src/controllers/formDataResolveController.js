...
const performJsonPathResolution = (key, value, dataResolveContext, {isImage, imageType}) => {
    try {
...
const handleDefaultValueExpressions = (component, dataResolveContext) => {
    ...
    if (component.type === 'content') {
        if (component.tags && component.tags.find( t => t === 'image')) {
            const imageType = component.properties ? (component.properties['imageType'] ? component.properties['imageType']: 'png'): 'png';
            const isImage = true;
            component.html = performJsonPathResolution(component.key, component.html, dataResolveContext, {isImage, imageType});
        }
    }
};
